...
void loadDefaultSettings() {
  ...
  // Tiltott idő megtartása default technológiai beállításnál.
...
const char* autoDisplayState(uint8_t zone) {
  ...
  if ((az->step == AUTO_FILL || az->step == AUTO_DRAIN) &&
      otherZoneInCommonOperation(zone)) {
    return "VARAKOZAS";
  }
...
bool otherZoneInCommonOperation(uint8_t zone) {
  if (zone == 1) {
    return (g_autoZ2.step == AUTO_FILL || g_autoZ2.step == AUTO_DRAIN);
  }

  if (zone == 2) {
    return (g_autoZ1.step == AUTO_FILL || g_autoZ1.step == AUTO_DRAIN);
  }
...
void runAutoZone(uint8_t zone) {
  ...
  if (az->step == AUTO_FILL) {
    if (otherZoneInCommonOperation(zone)) {
      // Másik zóna éppen tölt/leereszt, ezért ez vár.
      return;
    }
...
void writeMainStatusToHMI() {
  ...
  nexSetText("tZ1Active", g_settings.z1.enabled ? "AKTIV" : "KI");
  nexSetText("tZ2Active", g_settings.z2.enabled ? "AKTIV" : "KI");

  nexSetText("tZ1State", autoDisplayState(1));
  nexSetText("tZ2State", autoDisplayState(2));

  nexSetText("tZ1Ibc", String(g_settings.z1.tank).c_str());
  nexSetText("tZ2Ibc", String(g_settings.z2.tank).c_str());

  nexSetText("tZ1Full", zoneFull(1) ? "TELE" : "NEM TELE");
  nexSetText("tZ2Full", zoneFull(2) ? "TELE" : "NEM TELE");
...
void setup() {
  ...
  Serial.println("Indoor Garden - V2_3_AUTO_MVP_240526 indul");
